...
def object_list(initial_list):
    ...
    for index, element in enumerate(initial_list): # Each index changes from a series of numbers to a list that contains each number
        white_space_removed = element.split()
        print(white_space_removed[0] + white_space_removed[1] + white_space_removed[2])
        new_item = item(white_space_removed[0], white_space_removed[1], white_space_removed[2])
